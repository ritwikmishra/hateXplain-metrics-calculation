...
def tokenize_word_ritwik(text):
    ...
    for text_i in text:
        ...
        ## ------------------- OUTDATED BECAUSE OF ROBERTA TOKENIZER
        ...
        encodings = tokenizer(text_i,add_special_tokens=False) # turning off special tokens because it will be included (if required)
        input_ids_i = encodings['input_ids']
        word_ids_i = encodings.word_ids()
        num_words = len(set(word_ids_i))
        if model_parameter['token_ids_way']==1: # first
            i2 = []
            for x in range(num_words):
                i2.append(input_ids_i[word_ids_i.index(x)])
            input_ids_i = i2
            word_break_i = [1]*num_words
        elif model_parameter['token_ids_way']==2: # last
            i2 = []
            for x in range(num_words):
                i2.append(input_ids_i[len(word_ids_i)-word_ids_i[::-1].index(x)-1])
            input_ids_i = i2
            word_break_i = [1]*num_words
        elif model_parameter['token_ids_way']==3:
            # print()
            # print(text_i)
            # print(word_ids_i)
            # print(len(text_i.split()))
            word_break_i = [0]*num_words
            for x in word_ids_i:
                word_break_i[x]+=1
        else:
            raise 'Error'
...
def tokenize_my_sent_without_stanza(text):
    if not isinstance(text, str):
        raise TypeError("Input text must be a string.")

    tokenized_text_i = []
    for word in text.split():
        if len(tokenizer.tokenize(word)) > 0:
            tokenized_text_i.append(word)
    return ' '.join(tokenized_text_i)
...
class MODEL(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None, output_hidden_states=None):
        ...
        text = [tokenize_my_sent_without_stanza(tokenizer.convert_tokens_to_string(tokenizer.convert_ids_to_tokens(x))) for x in input_ids]
        input_ids, attention_mask, word_break, word_break_len = tokenize_word_ritwik(text)
        input_ids, attention_mask, word_break, word_break_len = torch.tensor(input_ids), torch.tensor(attention_mask),  torch.tensor(word_break), torch.tensor(word_break_len)
        input_ids, attention_mask, word_level, word_level_len = input_ids.to(device), attention_mask.to(device),  word_break.to(device), word_break_len.to(device)
    ...
    def predict(self, text): #set is_callig_for_lime2_shap_score=True if calling model.predict for lime1/lime2/shap relevance scores 
      ...
      #get prediction from model
        with torch.no_grad():
            if args.method == 'lime2' or args.method == 'shap':
                ...
                return logits, first_fc_layer_emb, word_level_embedding_flat
       

        logits = output.logits
        # Compute the label probabilities from the logits tensor
...
#   "========================LIME EXPLANATIONS IN ERASER FORMAT=========================="
    ...
    def run_lime(test_df):
        ...
        for i in tqdm(range(test_df.shape[0]), desc='getting lime scores...'):
            if False: #i >= 0:
                ...
                all_labels.append(label)
                all_output_proba.append( {'non-toxic':non_toxic_proba, 'toxic': toxic_proba})
                all_lime_scores.append([random.random()]*len(test_df.iloc[i]['commentText'].split()))
                all_post_tokens_cleaned.append(test_df.iloc[i]['commentText'].split())
                continue
            text = test_df.iloc[i]['commentText']


            text_tokens = text.split()
            if args.method=='lime1':
            ...
            #================RITWIK'S WAY OF LIME SCORES CALCULATIONS================
            elif args.method =='lime2':
                bench = Benchmark(model, tokenizer, explainers=[LIMEExplainer(model, tokenizer)])

                text = tokenize_my_sent_without_stanza(text)
                _, _, word_break, _ = tokenize_word_ritwik([text])
                l = len(text.split())
                ...
                # print(df)
                df2 = pd.DataFrame([],columns=['Token']+text.split())
                ...
                # print(df2)
                # input('wait')
                lime_score_list = list(df2.iloc[0])[1:]
            ...
            if len(lime_score_list) != len(text_tokens):
                ...
                print('len(lime_score_list):' ,len(lime_score_list))
                print(text)
                print(text.split())
                print(text_tokens)
                print(df2)
    ...
    # Set the path of the output file
    output_file_path = os.path.join(dir_path, model_name + '-dataSplit'+str(args.split)+ '-'+ args.method.upper()+'-' + 'faithfullness_filtering-' + args.faithfullness_filtering + '.json')
    ...
    def run_shap(test_df):
        ...
        for i in tqdm(range(test_df.shape[0]), desc='getting shap scores...'):
            if False: #i >= 3:
                ...
                all_labels.append(label)
                all_output_proba.append( {'non-toxic':non_toxic_proba, 'toxic': toxic_proba})
                all_shap_scores.append([random.random()]*len(test_df.iloc[i]['commentText'].split()))
                all_post_tokens_cleaned.append(test_df.iloc[i]['commentText'].split())
                continue
            text = test_df.iloc[i]['commentText']
            ...
            for i,row in df.iterrows():
                ...
                for offset in word_break:
                    ml = list(row[df.columns[idx:idx+offset]])
                    # print(df.columns[idx:idx+offset])
                    try:
                        assert len(ml)!=0
                    except:
                        print()
            ...
            # print(df2)
            # input('wait')
            shap_score_list = list(df2.iloc[0])[1:]
            # print(shap_score_list)
 
            if len(shap_score_list) != len(text_tokens):
                print('\nError\nlen(text_tokens), ', len(text_tokens))
                print('len(shap_score_list):' ,len(shap_score_list))
                print(text)
                print(text.split())
                print(text_tokens)
                print(df2)
